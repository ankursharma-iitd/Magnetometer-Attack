...
function [savGolSegments X_m Y_m] = test_new2(eegSignal, time,A1,F1, length_signal, fs, winlength, EEGSignal1_42sec)
...
for win = 1:numWin-1
    ...
    currFreq = signal2((win-1)*winNumElem + 1 : win*winNumElem);
    nextFreq = signal2(win*winNumElem + 1: (win+1)*winNumElemNext);
      disp('blehhhhh start');
      disp(adif(nextWin));
      disp(adif(currWin));
      if(F1 == 0) 
          ...
          if(A1 == 0)
             btt =(F1*(abs(fdif(nextFreq) - fdif(currFreq)))); 
          else
              x = abs(adif(nextWin) - adif(currWin));
              y = abs(fdif(nextFreq) - fdif(currFreq));
              disp('X down');
              disp(x);
              disp('Y down');
              disp(y);
      ...
      Y_m = [Y_m (abs(fdif(nextFreq) - fdif(currFreq)))];
      G_m(win) = btt;
      disp(win);
      disp(btt);
      disp(G_m(win));
